...
var app = {
    ...
    launch_map_app: function () {
        if (!
                map.hasOwnProperty('directionsResult') && map.directionsResult.hasOwnProperty('request') && typeof map.directionsResult.request === 'object' && typeof launchnavigator === 'object') {
            return false;
        }
...
var backboneInit = function () {
    ...
    app.utils.templates.load(["MenuView", "HomeView", "SignupView", "DashboardView", "ForgotView", "RiderView", "RiderWithETAView", "WaitDriverView", "OfferListItemView", "Request_ride_view", 'RiderWaitPickupView', 'RequestListItemView', "View_riders_view"], function () {
        app.router = new app.routers.AppRouter();
...
//
            onSuccess: function (position) {
                ...
                if (_.isObject(app.cuser)) {
                    app.cuser.save($.extend({lat: current_pos.latitude, lng: current_pos.longitude, status: app.status}, extra_param), {
                        patch: true,
                        forceRefresh: true
                    });
                }
